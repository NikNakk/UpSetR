...
Remove <- function(data, start_col, end_col, sets) {
  ...
  colnames(temp_data[ ,!(colnames(temp_data) %in% sets), drop = FALSE])
}

## Removes unwanted sets from data
Wanted <- function(data, unwanted_sets) {
  (data[ ,!(colnames(data) %in% unwanted_sets), drop = FALSE])
}
...
OverlayEdit <- function(data1, data2, start_col, num_sets, intersects, exp, inter_color) {
  ...
  unwanted <- colnames(set_cols[ ,!(colnames(set_cols) %in% intersects), drop = FALSE])
  temp_data <- (temp_data[ ,!(colnames(data1) %in% unwanted), drop = FALSE])
